...
export function buildDemoGuide(p,index,isUser){
 ...
 return {company:companyStories[p.id],personId:p.id,momentIndex:index,name:p.name,time:m.time,title:m.title,why:m.body,perspective:isUser?'User view':'Admin view',job:p.job,story:p.intro,
  ...(p.id==='sam'?{liveUrl:new URL('./desk.html?stage=1',import.meta.url).href,liveLabel:'Harborline Front Desk',liveKind:'front desk demo app',demoPreview:true}:{}),
  ...
  highlights:highlights[p.id][index],admin, user:p.id==='sam'?[
   s('Open Harborline Front Desk on screen 2 and inspect the sample arrivals, rooms and guest service queue.','Show the work a night auditor performs in a named application session on a shared PC. The app uses synthetic data and does not authenticate with Entra or a PMS.','The demo-data label stays visible. For actual device and Microsoft sign-in evidence, open the protected kiosk VM separately.'),
   ...
   s('Create or finish one guest service request, then choose End shift.','The next worker must not inherit the previous application screen or its in-memory demo state.','The app removes the previous view and offers a fresh demo shift; separately prove Windows/browser cleanup on hl-kiosk-01.'),
   s('On the protected kiosk VM, inspect the actual Windows session, browser sign-in and policy evidence.','The public front desk app does not prove Microsoft authentication, Conditional Access or device cleanup.','Match Sam’s real sign-in and kiosk configuration in the protected lab before claiming an identity outcome.')
  ]:[
