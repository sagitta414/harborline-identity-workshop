...
const guides={
 ...
 sofia:[
  ...
  [[24],[
   ...
   s('Open the protected kiosk desktop, then launch the signed-in Harborline Front Desk in its browser.','The application initiates Entra sign-in; the public sample app remains a separate preview.','Start the passkey demonstration in a top-level browser tab, not inside the workshop frame.')
  ]],
  [[3,24],[
...
export function buildDemoGuide(p,index,isUser){
 ...
 return {company:companyStories[p.id],personId:p.id,momentIndex:index,name:p.name,time:m.time,title:m.title,why:m.body,perspective:isUser?'User view':'Admin view',job:p.job,story:p.intro,
  ...
  highlights:highlights[p.id][index],admin, user:p.id==='sam'?[
   s('From a fresh top-level browser profile, open the signed-in Harborline Front Desk. Start at the app, continue to Microsoft Entra, choose Sam’s registered passkey when offered, then watch the browser return to the app.','This shows the app-initiated sign-in and the handoff back to a named Front Desk session.','Use the protected kiosk browser for device claims. If Entra does not offer a passkey, do not call the result a passkey sign-in.'),
