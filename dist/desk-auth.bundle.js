...
function guestRow(g) {
  return `<button type="button" class="row" data-guest="${esc(g.id)}"><span class="row-icon">${esc(g.initials)}</span><span class="row-main"><strong>${esc(g.name)}</strong><small>${esc(g.id)} \xB7 ${esc(g.type)} \xB7 ${g.nights} ${g.nights === 1 ? "night" : "nights"}</small></span><span class="row-end"><span class="status ${mark(g.status)}">${esc(g.status)}</span><small>${g.room ? "Room " + esc(g.room) : esc(g.time)}</small></span></button>`;
}
function activity() {
...
function detail() {
  ...
  if (drawer.kind === "guest") {
    ...
    if (!g) return "";
    const ready = data.rooms.filter((r) => r.type === g.type && r.status === "Ready");
    return `<div class="sheet-backdrop" data-action="close-sheet"><aside class="sheet" role="dialog" aria-modal="true" aria-label="Guest details"><div class="sheet-head"><div><div class="eyebrow">Reservation ${esc(g.id)}</div><h2>${esc(g.name)}</h2><p>${esc(g.status)} \xB7 ${esc(g.party)} ${g.party === 1 ? "guest" : "guests"} \xB7 ${esc(g.type)}</p></div><button class="icon-btn" type="button" data-action="close-sheet" aria-label="Close details">\xD7</button></div><dl class="details"><div><dt>Stay</dt><dd>${g.nights} ${g.nights === 1 ? "night" : "nights"}</dd></div><div><dt>Room</dt><dd>${g.room ? esc(g.room) : "Unassigned"}</dd></div><div><dt>Arrival</dt><dd>${esc(g.time)}</dd></div><div><dt>Status</dt><dd>${esc(g.status)}</dd></div></dl><div class="notice">Sample reservation. No real guest record or hotel system is connected.</div>${g.note ? `<p class="note-inline"><strong>Desk note:</strong> ${esc(g.note)}</p>` : ""}${g.status === "Arriving" ? `<label class="field" for="assign-room">Assign a ready ${esc(g.type)} room<select id="assign-room"><option value="">Choose a room</option>${ready.map((r) => `<option value="${r.number}">${r.number} \xB7 ${esc(r.floor)}</option>`).join("")}</select></label>${ready.length ? "" : '<div class="alert">No matching room is ready. Update the room board after housekeeping confirms readiness.</div>'}<div class="sheet-actions"><button class="btn primary" type="button" data-action="checkin" ${ready.length ? "" : "disabled"}>Confirm check-in</button><button class="btn" type="button" data-view="rooms">Room board</button></div>` : `<div class="sheet-actions"><button class="btn danger" type="button" data-action="checkout">Check out guest</button><button class="btn" type="button" data-action="new-task" data-prefill-room="${esc(g.room)}">Service request</button></div>`}</aside></div>`;
  }
  if (drawer.kind === "room") {
    const r = data.rooms.find((x) => x.number === drawer.id), g = data.guests.find((x) => x.room === r.number && x.status === "In house");
    return `<div class="sheet-backdrop" data-action="close-sheet"><aside class="sheet" role="dialog" aria-modal="true" aria-label="Room details"><div class="sheet-head"><div><div class="eyebrow">${esc(r.floor)} \xB7 ${esc(r.type)}</div><h2>Room ${esc(r.number)}</h2><p><span class="status ${mark(r.status)}">${esc(r.status)}</span></p></div><button class="icon-btn" type="button" data-action="close-sheet" aria-label="Close details">\xD7</button></div><div class="notice">Readiness is simulated here. Confirm housekeeping and guest status in the real property system before assigning a room.</div>${g ? `<p class="note-inline">Currently assigned to ${esc(g.name)}.</p>` : ""}<div class="sheet-actions">${r.status === "Cleaning" ? `<button class="btn primary" type="button" data-action="ready-room">Mark ready</button>` : r.status === "Ready" ? `<button class="btn" type="button" data-action="clean-room">Send to cleaning</button>` : '<button class="btn" type="button" disabled>Occupied</button>'}</div></aside></div>`;
  }
  return `<div class="sheet-backdrop" data-action="close-sheet"><aside class="sheet" role="dialog" aria-modal="true" aria-label="New service request"><div class="sheet-head"><div><div class="eyebrow">Guest service</div><h2>New request</h2><p>Send a task to the property team.</p></div><button class="icon-btn" type="button" data-action="close-sheet" aria-label="Close details">\xD7</button></div><form id="task-form"><label class="field">Room number<input name="room" inputmode="numeric" maxlength="4" required value="${esc(drawer.room || "")}" placeholder="e.g. 608"></label><label class="field">Team<select name="category"><option>Housekeeping</option><option>Maintenance</option><option>Guest services</option></select></label><label class="field">What needs attention?<textarea name="note" maxlength="240" required placeholder="Describe the request"></textarea></label><button class="btn primary full" type="submit">Add to service queue</button></form></aside></div>`;
}
function lockScreen() {
...
var init_desk = __esm({
  "dist/desk.js"() {
    ...
    ico = (name3) => `<svg aria-hidden="true" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${icons[name3]}</svg>`;
    ...
    app.addEventListener("click", (event) => {
      ...
      if (button.dataset.complete) {
        const task = data.tasks.find((x) => x.id === Number(button.dataset.complete));
        if (task) {
          task.status = "Done";
          record("Request completed", `${task.category} \xB7 Room ${task.room}`);
          render();
      ...
      if (action === "checkin") {
        ...
        if (!g || g.status !== "Arriving" || !room || room.status !== "Ready" || room.type !== g.type) {
        ...
        drawer = null;
        render();
        notify(`Check-in recorded in demo \xB7 Room ${room.number}`);
        return;
      }
      if (action === "checkout") {
        const g = data.guests.find((x) => x.id === drawer?.id);
      ...
      if (action === "ready-room" || action === "clean-room") {
        ...
        record(`Room ${room.number} ${room.status.toLowerCase()}`, "Housekeeping status updated in demo");
        drawer = null;
        render();
      ...
      if (action === "new-shift") {
        ...
        data = seed();
        view = "overview";
        record("Fresh shift started", `${operator} \xB7 sample records reset`);
        render();
    ...
    app.addEventListener("input", (event) => {
      if (event.target.id === "guest-search") {
        ...
        const guests = data.guests.filter((g) => g.status !== "Departed" && `${g.name} ${g.id} ${g.room || ""}`.toLowerCase().includes(search.toLowerCase()));
        list.innerHTML = guests.length ? guests.map(guestRow).join("") : '<div class="empty">No matching sample guests.</div>';
    ...
    app.addEventListener("submit", (event) => {
      ...
      const values = new FormData(event.target), room = String(values.get("room") || "").trim(), category = String(values.get("category") || ""), note = String(values.get("note") || "").trim();
      if (!/^\d{3,4}$/.test(room)) {
      ...
      data.tasks.unshift({ id: Date.now(), room, category, note, status: "Open", owner: category === "Housekeeping" ? "Rooms team" : category === "Maintenance" ? "Engineering" : "Front desk" });
      record("Service request added", `${category} \xB7 Room ${room}`);
      drawer = null;
      view = "service";
      render();
...
function buildTenantProfile(homeAccountId, localAccountId, tenantId2, nativeAccountId, idTokenClaims) {
  if (idTokenClaims) {
...
var UrlString = class _UrlString {
  ...
  replaceTenantPath(tenantId2) {
    ...
    if (tenantId2 && pathArray.length !== 0 && (pathArray[0] === AADAuthority.COMMON || pathArray[0] === AADAuthority.ORGANIZATIONS)) {
...
function createAccessTokenEntity(homeAccountId, environment, accessToken, clientId2, tenantId2, scopes, expiresOn, extExpiresOn, base64Decode2, correlationId, refreshOn, tokenType, userAssertionHash, keyId, additionalCacheKeyComponents) {
  const atEntity = {
    homeAccountId,
  ...
  replacePath(urlString) {
    ...
    currentAuthorityParts.forEach((currentPart, index) => {
      ...
      if (index === 0 && this.canReplaceTenant(cachedAuthorityUrlComponents)) {
        const tenantId2 = new UrlString(this.metadata.authorization_endpoint, this.correlationId).getUrlComponents().PathSegments[0];
        if (cachedPart !== tenantId2) {
          this.logger.verbose(`1q3g2x ${cachedPart} ${tenantId2}`, this.correlationId);
...
function getTenantFromAuthorityString(authority, correlationId) {
  const authorityUrl = new UrlString(authority, correlationId);
  const authorityUrlComponents = authorityUrl.getUrlComponents();
  const tenantId2 = authorityUrlComponents.PathSegments.slice(-1)[0]?.toLowerCase();
  switch (tenantId2) {
...
var DEFAULT_CRYPTO_IMPLEMENTATION = {
  ...
  async signTokenBindingJwt(header2, payload, kid, correlationId) {
    throw createClientAuthError(methodNotImplemented, correlationId);
  },
  async hashString() {
    throw createClientAuthError(methodNotImplemented, "");
...
function markAsRecentlyUsed(correlationId, data2) {
  correlationCache.delete(correlationId);
  correlationCache.set(correlationId, data2);
}
function addLogToCache(correlationId, loggedMessage) {
  const currentTime = Date.now();
  let data2 = correlationCache.get(correlationId);
  if (data2) {
    markAsRecentlyUsed(correlationId, data2);
  } else {
    data2 = { logs: [], firstEventTime: currentTime };
    correlationCache.set(correlationId, data2);
  ...
  data2.logs.push({
    ...loggedMessage,
    milliseconds: currentTime - data2.firstEventTime
  });
  if (data2.logs.length > MAX_LOGS_PER_CORRELATION) {
    data2.logs.shift();
  }
}
...
var CacheManager = class {
  constructor(clientId2, cryptoImpl, logger, performanceClient, staticAuthorityOptions, tokenBindingKeyManager) {
   ...
   * @param clientId
   */
  matchClientId(entity, clientId2) {
    return !!(entity.clientId && clientId2 === entity.clientId);
  }
  /**
...
var ResponseHandler = class _ResponseHandler {
  constructor(clientId2, cacheStorage, cryptoObj, logger, performanceClient, serializableCache, persistencePlugin, tokenBindingKeyManager = DEFAULT_TOKEN_BINDING_KEY_MANAGER) {
    this.clientId = clientId2;
...
function buildAccountToCache(cacheStorage, authority, homeAccountId, base64Decode2, correlationId, idTokenClaims, clientInfo, environment, claimsTenantId, authCodePayload, nativeAccountId, logger, performanceClient) {
  ...
  if (tenantId2 && !tenantProfiles.find((tenantProfile) => {
    ...
    const newTenantProfile = buildTenantProfile(homeAccountId, baseAccount.localAccountId, tenantId2, nativeAccountId, idTokenClaims);
...
async function getClientAssertion(clientAssertion, clientId2, tokenEndpoint, fmiPath) {
...
function getRequestThumbprint(clientId2, request, homeAccountId) {
  return {
    clientId: clientId2,
...
var ThrottlingUtils = class _ThrottlingUtils {
  ...
  static removeThrottle(cacheManager, clientId2, request, homeAccountIdentifier) {
    const thumbprint = getRequestThumbprint(clientId2, request, homeAccountIdentifier);
...
function createTokenQueryParameters(request, clientId2, redirectUri2, performanceClient) {
  const parameters = /* @__PURE__ */ new Map();
  if (request.embeddedClientId) {
    addBrokerParameters(parameters, clientId2, redirectUri2);
  }
...
async function sign(key, data2, algorithm) {
  return window.crypto.subtle.sign(algorithm, key, data2);
}
...
async function decryptEarResponse(earJwk, earJwe) {
  ...
  try {
    const header2 = new TextEncoder().encode(earJweParts[0]);
...
var CryptoOps = class {
  ...
  async signTokenBindingJwt(header2, payload, kid, correlationId) {
    ...
    try {
      ...
      await this.validateTokenBindingJwtHeaderKey(header2, kid, correlationId);
      telemetry = this.tokenBindingKeyManager.getTokenBindingKeyTelemetry(cachedKeyPair, header2.alg);
      const signingAlgorithm = this.getTokenBindingKeySigningAlgorithm(cachedKeyPair, header2.alg, correlationId);
      const tokenString = `${urlEncode(JSON.stringify(header2))}.${urlEncode(JSON.stringify(payload))}`;
  ...
  async validateTokenBindingJwtHeaderKey(header2, kid, correlationId) {
    if (!header2.jwk) {
      return;
    }
    const headerKeyId = await computeJwkThumbprint(header2.jwk, correlationId);
    if (headerKeyId !== kid) {
...
function getTokenKeysCacheKey(clientId2, schema = CREDENTIAL_SCHEMA_VERSION) {
  if (schema < 1) {
    return `${PREFIX}.${TOKEN_KEYS}.${clientId2}`;
  }
  return `${PREFIX}.${schema}.${TOKEN_KEYS}.${clientId2}`;
}
...
function getTokenKeys(clientId2, storage, schemaVersion) {
  const item = storage.getItem(getTokenKeysCacheKey(clientId2, schemaVersion));
  if (item) {
...
function isEncrypted(data2) {
  return data2.hasOwnProperty("id") && data2.hasOwnProperty("nonce") && data2.hasOwnProperty("data");
}
...
var LocalStorage = class {
  constructor(clientId2, logger, performanceClient) {
  ...
  async decryptData(key, data2, correlationId) {
    if (!this.initialized || !this.encryptionCookie) {
      throw createBrowserAuthError(uninitializedPublicClientApplication, "");
    }
    if (data2.id !== this.encryptionCookie.id) {
      this.performanceClient.incrementFields({ encryptedCacheExpiredCount: 1 }, correlationId);
      return null;
    }
    const decryptedData = await invokeAsync(decrypt, Decrypt, this.logger, this.performanceClient, correlationId)(this.encryptionCookie.key, data2.nonce, this.getContext(key), data2.data);
    if (!decryptedData) {
      return null;
    }
    try {
      return {
        ...
        lastUpdatedAt: data2.lastUpdatedAt
      };
    } catch (e) {
  ...
  async setUserData(key, value, correlationId, timestamp, kmsi) {
    ...
    if (kmsi) {
      ...
      const { data: data2, nonce } = await invokeAsync(encrypt, Encrypt, this.logger, this.performanceClient, correlationId)(this.encryptionCookie.key, value, this.getContext(key));
      const encryptedData = {
...
var BrowserCacheManager = class extends CacheManager {
  constructor(clientId2, cacheConfig, cryptoImpl, logger, performanceClient, eventHandler, staticAuthorityOptions, tokenBindingKeyManager = DEFAULT_TOKEN_BINDING_KEY_MANAGER) {
    super(clientId2, cryptoImpl, logger, performanceClient, staticAuthorityOptions, tokenBindingKeyManager);
    this.cacheConfig = cacheConfig;
    this.logger = logger;
    this.internalStorage = new MemoryStorage();
    this.browserStorage = getStorageImplementation(clientId2, cacheConfig.cacheLocation, logger, performanceClient);
    this.temporaryCacheStorage = getStorageImplementation(clientId2, BrowserCacheLocation.SessionStorage, logger, performanceClient);
  ...
  async migrateIdTokens(credentialSchema, accountSchema, correlationId) {
    ...
    for (const idTokenKey of [...credentialKeysToMigrate.idToken]) {
      ...
      if (!currentIdToken || oldSchemaData.lastUpdatedAt > currentIdToken.lastUpdatedAt && (oldTokenHasSignInState || !currentTokenHasSignInState)) {
        ...
        const tenantId2 = getTenantIdFromIdTokenClaims(claims) || account.realm;
        if (tenantId2 && !tenantProfiles.find((tenantProfile) => {
          return tenantProfile.tenantId === tenantId2;
        })) {
          const newTenantProfile = buildTenantProfile(account.homeAccountId, account.localAccountId, tenantId2, account.nativeAccountId, claims);
          tenantProfiles.push(newTenantProfile);
        }
      ...
      try {
        ...
        if (i > 0) {
          for (let schemaVersion = 0; schemaVersion <= CREDENTIAL_SCHEMA_VERSION; schemaVersion++) {
            const startIndex = tokenKeysCount.slice(0, schemaVersion).reduce((sum, count2) => sum + count2, 0);
            if (startIndex >= i) {
              break;
            }
  ...
  isInteractionInProgress(matchClientId) {
    const clientId2 = this.getInteractionInProgress()?.clientId;
    if (matchClientId) {
      return clientId2 === this.clientId;
    } else {
      return !!clientId2;
    }
  }
...
function getStorageImplementation(clientId2, cacheLocation, logger, performanceClient) {
  try {
    switch (cacheLocation) {
      case BrowserCacheLocation.LocalStorage:
        return new LocalStorage(clientId2, logger, performanceClient);
...
var DEFAULT_BROWSER_CACHE_MANAGER = (clientId2, logger, performanceClient, eventHandler, staticAuthorityOptions, tokenBindingKeyManager = DEFAULT_TOKEN_BINDING_KEY_MANAGER) => {
  const cacheOptions = {
    cacheLocation: BrowserCacheLocation.MemoryStorage,
    cacheRetentionDays: 5
  };
  return new BrowserCacheManager(clientId2, cacheOptions, DEFAULT_CRYPTO_IMPLEMENTATION, logger, performanceClient, eventHandler, staticAuthorityOptions, tokenBindingKeyManager);
};
...
function getRedirectUri(requestRedirectUri, clientConfigRedirectUri, logger, correlationId) {
  ...
  const redirectUri2 = requestRedirectUri || clientConfigRedirectUri || "";
  return UrlString.getAbsoluteUrl(redirectUri2, getCurrentUri(), correlationId);
}
function initializeServerTelemetryManager(apiId, clientId2, correlationId, browserStorage, logger, forceRefresh, enabled = true) {
...
async function initializeAuthorizationRequest(request, interactionType, config, browserCrypto, browserStorage, logger, performanceClient, correlationId) {
  const redirectUri2 = getRedirectUri(request.redirectUri, config.auth.redirectUri, logger, correlationId);
  if (new URL(redirectUri2).origin !== new URL(window.location.href).origin) {
...
async function waitForPopupRelayResponse(timeoutMs, logger, request, popupWindow, performanceClient) {
  return new Promise((resolve, reject) => {
    ...
    const onMessage = (event) => {
      ...
      if (!data2 || data2.type !== POPUP_RELAY_RESPONSE_TYPE || data2.id !== expectedId) {
        return;
      }
      if (settled) {
        return;
      ...
      cleanup();
      if (data2.error) {
        reject(createBrowserAuthError(data2.error, correlationId));
      } else if (data2.payload) {
        resolve(data2.payload);
      } else {
        reject(createBrowserAuthError(redirectBridgeEmptyResponse, correlationId));
      }
    };
...
var PlatformAuthInteractionClient = class extends BaseInteractionClient {
  ...
  async acquireTokenRedirect(request, rootMeasurement, options) {
    ...
    const redirectUri2 = navigateToLoginRequestUrl ? UrlString.getAbsoluteUrl(request.redirectStartPage || window.location.href, getCurrentUri(), this.correlationId) : getRedirectUri(request.redirectUri, this.config.auth.redirectUri, this.logger, this.correlationId);
    rootMeasurement.end({ success: true });
    await this.navigationClient.navigateExternal(redirectUri2, navigationOptions);
  }
  /**
   ...
   * @param reqTimestamp
   */
  async cacheNativeTokens(response, request, homeAccountIdentifier, idTokenClaims, tenantId2, reqTimestamp, environment, storeInCache) {
    ...
    const cachedAccessToken = CacheHelpers_exports.createAccessTokenEntity(homeAccountIdentifier, environment, response.access_token, request.clientId, idTokenClaims.tid || tenantId2, responseScopes.printScopes(), tokenExpirationSeconds, 0, base64Decode, request.correlationId, void 0, request.tokenType, void 0, request.keyId, additionalCacheKeyComponents);
...
var PlatformAuthDOMHandler = class _PlatformAuthDOMHandler {
  ...
  initializePlatformDOMRequest(request) {
    ...
    const { accountId, clientId: clientId2, authority, scope, redirectUri: redirectUri2, correlationId, state, extraParameters, ...remainingProperties } = request;
...
var StandardController = class _StandardController {
  ...
  logMultipleInstances(performanceEvent, correlationId) {
    const clientId2 = this.config.auth.clientId;
    if (!window)
      return;
    ...
    window.msal.clientIds.push(clientId2);
    collectInstanceStats(clientId2, performanceEvent, this.logger, correlationId);
  }
};
...
var root = document.querySelector("#desk-app");
var escape = (value) => String(value ?? "").replace(/[&<>"']/g, (char) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[char]);
var backLink = new URL("./desk.html", location.href).href;
var msal;
function gate(message = "Your Entra account opens a named Front Desk session.", error = "") {
  root.innerHTML = `<div class="auth-page"><header class="auth-top"><div class="auth-brand"><img src="./assets/harborline-mark.svg" alt=""><span><strong>Harborline</strong><small>FRONT DESK</small></span></div><span class="auth-environment">WORKFORCE IDENTITY DEMO</span></header><main class="auth-main"><section class="auth-copy"><div class="eyebrow">One sign-in \xB7 one application session</div><h1>Start at the desk.<br><em>Arrive as yourself.</em></h1><p>Open Harborline Front Desk with your assigned Microsoft Entra identity. The browser takes you to Entra for sign-in and returns here after the tenant accepts it.</p><ol class="auth-flow"><li><b>01</b><span><strong>Open the application</strong><small>The app requests a Workforce tenant session.</small></span></li><li><b>02</b><span><strong>Sign in at Microsoft Entra</strong><small>Choose a registered passkey if Entra offers it.</small></span></li><li><b>03</b><span><strong>Return to Front Desk</strong><small>The signed-in account appears in the app header.</small></span></li></ol></section><section class="auth-card" aria-label="Front Desk sign-in"><img src="./assets/harborline-mark.svg" alt=""><span class="auth-card-kicker">HARBORLINE \xB7 BALLARD LOCKS HOUSE</span><h2>Your shift starts here.</h2><p>${escape(message)}</p>${error ? `<div class="auth-error" role="alert">${escape(error)}</div>` : ""}${canSignIn ? `<button class="btn primary auth-button" id="start-signin" type="button" ${msal ? "" : "disabled"}>Sign in with Microsoft Entra <span aria-hidden="true">\u2197</span></button>` : `<a class="btn primary auth-button" href="${liveEntry}">Open the signed-in app <span aria-hidden="true">\u2197</span></a>`}<a class="auth-preview-link" href="${backLink}">View the public sample app</a><div class="auth-clarity">Passkey availability is controlled by the tenant and the account. This page does not collect your password or passkey.</div></section></main><footer class="auth-footer"><span>MajorKey identity workshop \xB7 Harborline reference lab</span><span>Sample hotel data \xB7 no PMS connection</span></footer></div>`;
  root.querySelector("#start-signin")?.addEventListener("click", async () => {
    try {
      await msal.loginRedirect({ scopes: ["openid", "profile"] });
    } catch (error2) {
      console.error("Front Desk sign-in could not start", error2);
      gate("Microsoft Entra sign-in could not start.", "Try again in a fresh top-level browser tab.");
    }
  });
}
async function start() {
  if (!canSignIn) {
    gate("The signed-in application opens on the Harborline Azure site. Continue there in a top-level browser tab.");
    return;
  }
  ...
  try {
    ...
    if (!token?.idToken) {
      gate("The previous session could not be confirmed. Sign in again.");
      return;
    }
