...
export function installDualStage(root,config,validateGateway,onUserFocus,onAdminFocus){
 ...
 let userFrame=null,aiFrame=null,stopUserFocus=null,stopAiFocus=null,userTimer=null,layout='both',userUrl=null,userSourceUrl=null,userSourceLabel='protected user desktop',userSourceKind='customer application',userIsLiveApp=false,userIsPreview=false,userPersonName='User',agentUrl=AGENTS+'/identity',activeSide='admin',currentGuideName=null;
 ...
 function setActive(side){
  ...
  if(side==='user'){activeTitle.textContent=`Screen 2 · ${userPersonName} experience`;activePurpose.textContent=userIsPreview?'Show the simulated front desk workflow; verify identity on the kiosk VM':userIsLiveApp?`Show the live ${userSourceKind} response and enforced outcome`:'Show the persona’s sign-in and desktop experience';}
 ...
 function connectUser(){
  if(userFrame||!userSourceUrl)return;const isLiveApp=userSourceUrl!==userUrl;if(isLiveApp&&!userIsPreview)root.dispatchEvent(new CustomEvent('stage-live-app-open',{detail:{label:userSourceLabel}}));userFrame=document.createElement('iframe');userFrame.id='user-session-frame';userFrame.title=userIsPreview?`${userSourceLabel} · simulated operations`:isLiveApp?`Live ${userSourceLabel}`:'Protected independent kiosk desktop for the demo persona';userFrame.tabIndex=0;stopUserFocus=watchRemoteFocus(userFrame,()=>{setActive('user');onUserFocus();});userFrame.referrerPolicy='no-referrer';userFrame.setAttribute('allow','fullscreen');userFrame.setAttribute('allowfullscreen','');userFrame.src=userSourceUrl;userWelcome.hidden=true;userFocus.disabled=false;userDisconnect.disabled=false;userStatus.textContent=isLiveApp?`Opening ${userSourceLabel}…`:'Opening protected gateway…';userFrame.addEventListener('load',()=>{clearTimeout(userTimer);userStatus.textContent=userIsPreview?'Interactive demo · synthetic data · kiosk sign-in separate':isLiveApp?`${userSourceLabel} opened · sign-in and API proof unverified`:'Gateway page opened · sign in inside';if(document.activeElement===userFrame&&layout!=='admin')focusSide('user');});userFrame.addEventListener('error',()=>{clearTimeout(userTimer);userStatus.textContent=isLiveApp?'Application unavailable · use Separate tab':'Gateway unavailable · try separate tab';});surfaces.user.append(userFrame);userTimer=setTimeout(()=>userStatus.textContent='Still waiting · try separate tab',20000);focusSide('user');
 }
 function disconnectUser(){clearTimeout(userTimer);stopUserFocus?.();stopUserFocus=null;userFrame?.remove();userFrame=null;userWelcome.hidden=false;userFocus.disabled=true;userDisconnect.disabled=true;userStatus.textContent=userIsPreview?'Demo view closed · in-memory state discarded':'View disconnected · Windows session may remain signed in';}
 function connectAi(){
 ...
 return {setActive,setLayout,connect:connectUser,connected:()=>Boolean(userFrame),focus:()=>focusSide(activeSide==='ai'?'ai':'user'),isFocused:()=>Boolean((userFrame&&document.activeElement===userFrame)||(aiFrame&&document.activeElement===aiFrame)),visible:()=>layout==='user'||layout==='both',setGuide(g){
  ...
  userSourceUrl=nextUserUrl;userSourceLabel=nextUserLabel;userSourceKind=g.liveKind||'customer application';userIsLiveApp=Boolean(g.liveUrl);userIsPreview=Boolean(g.demoPreview);
  userDirect.href=userSourceUrl||'';if(userIsPreview){const appTabUrl=new URL(userSourceUrl);appTabUrl.searchParams.delete('stage');userDirect.href=appTabUrl.href}userDirect.hidden=!userSourceUrl;
  const kioskLink=user.querySelector('#user-session-vm');kioskLink.hidden=!userIsPreview||!userUrl;kioskLink.href=userUrl||'';kioskLink.textContent='Kiosk VM ↗';const authLink=user.querySelector('#user-session-auth');authLink.hidden=!g.authUrl;authLink.href=g.authUrl||'';userDirect.textContent=userIsPreview?'App tab ↗':'Separate tab ↗';
  user.querySelector('[data-dual="user-connect"]').disabled=!userSourceUrl;
  userWelcome.querySelector('.stage-tag').textContent=userIsPreview?'INTERACTIVE APP DEMO · SYNTHETIC DATA':g.liveUrl?`${userSourceKind.toUpperCase()} · LINK CONFIGURED`:'INDEPENDENT LIVE SESSION';
  userWelcome.querySelector('h3').textContent=g.liveUrl?`Open ${userSourceLabel}.`:'The other side of the story.';
  userWelcome.querySelector('p').textContent=userIsPreview?'Work sample arrivals and rooms beside the Entra administrator view. Use the protected kiosk VM for actual device and sign-in proof.':g.liveUrl?`Run the real ${userSourceKind} beside the administrator tenant view.`:'Open the kiosk desktop and sign in with the persona’s Microsoft account in its browser.';
  userWelcome.querySelector('.session-note').textContent=userIsPreview?'This panel shows sample operations. Use Passkey → app in a separate tab for real Entra sign-in; confirm the method in sign-in logs. The kiosk VM proves device controls separately.':g.liveUrl?'Sign in inside the portal; verify the account and API result before presenting.':'Use the existing protected gateway sign-in. Windows credentials are requested inside the gateway.';
  userWelcome.querySelector('[data-dual="user-connect"]').textContent=g.liveUrl?`Open ${userSourceKind}`:'Open user session';
  userDisconnect.textContent=userIsPreview?'Close app':g.liveUrl?`Close ${userSourceKind}`:'Disconnect user';
  userStatus.textContent=userIsPreview?'Demo · VM separate':g.liveUrl?'Portal link configured · sign-in unverified':'Not connected';
  userPersonName=g.name;user.querySelector('#stage-user-name').textContent=g.name;user.querySelector('#stage-user-purpose').textContent=userIsPreview?'Sample hotel operations · identity proof on protected kiosk VM':g.liveUrl?`Live ${g.liveKind||'application'} · show the sign-in and enforced outcome`:'Separate protected desktop · show the person’s experience';ai.querySelector('#stage-ai-name').textContent=g.name+' · AI copilot';
