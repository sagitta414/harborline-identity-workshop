...
assert.equal(new Set(people.map(p=>p.id)).size,8);
...
assert.equal((renderedHome.match(/workforce-model/g)||[]).length,6);
assert.equal((renderedHome.match(/journey-card-ai/g)||[]).length,10);
